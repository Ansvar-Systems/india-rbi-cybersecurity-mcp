# Multi-stage build for india-rbi-cybersecurity-mcp
...
ENV RBI_DB_PATH=/app/data/rbi.db

EXPOSE 9191

HEALTHCHECK --interval=30s --timeout=10s --start-period=10s --retries=3 \
  CMD node -e "fetch('http://localhost:9191/health').then(r=>r.ok?process.exit(0):process.exit(1)).catch(()=>process.exit(1))"
